...
public class Employee extends EmployeeSalary{
    ...
    private static int highest_salary(ArrayList<Employee> emp, String Company_Name){
        ...
        if(max_salary == -999.99F) System.out.println("\n\nNo employees of said company exist in database!!");
        else{
            System.out.println("\n\nList of employees with highest salary from "+Company_Name+" :~");
            for(i=0;i<emp.size();i++){
                if(emp.get(i).CompanyName.equalsIgnoreCase(Company_Name) && emp.get(i).NetSalary == max_salary){
    ...
    private static int display_menu(){
        ...
        System.out.println("Enter 3 to view the highest paid employee of a given company.");
        System.out.println("Enter 4 to view the lowest paid employee of a given company.");
